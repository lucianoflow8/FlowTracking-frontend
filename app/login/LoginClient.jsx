...
export default function LoginClient() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (error) {
        if (error.message?.toLowerCase().includes("email not confirmed") || error.status === 401) {
    ...
    <main className="min-h-screen bg-[#0B0B0D] text-white flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        <div className="mb-6 text-center">
          ...
          <p className="mt-1 text-white/60">Ingresá con tu correo y contraseña.</p>
        </div>

        <form onSubmit={onSubmit} className="rounded-2xl border border-white/10 bg-white/[0.02] p-5 backdrop-blur">
          ...
          <label className="block text-sm text-white/70 mt-4 mb-1">Contraseña</label>
          <input
            type="password"
          ...
          {msg.text ? (
            <p className={`mt-3 text-sm ${msg.type === "error" ? "text-red-400" : msg.type === "success" ? "text-emerald-400" : "text-white/70"}`}>
              {msg.text}
            </p>
          ) : null}
